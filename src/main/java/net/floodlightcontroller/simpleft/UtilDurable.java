...
public class UtilDurable {
    ...
    public OFRoleReply setSwitchRole(IOFSwitch sw, OFControllerRole role) {
        try {
            ListenableFuture<OFRoleReply> future = sw
                    .writeRequest(sw.getOFFactory().buildRoleRequest().setGenerationId(U64.ZERO).setRole(role).build());
            return future.get(10, TimeUnit.SECONDS);

        } catch (Exception e) {
            if (sw == null) {
                logger.error("sw is null, cat not set switch role to {}", role.toString());
            } else {
