...
import tank.sdnos.clustermanager.ClusterInfo;
import tank.sdnos.clustermanager.ControllerRoleManager;
...
public class FT
    ...
    public void startUp(FloodlightModuleContext context) throws FloodlightModuleException {
        ...
        try {
            ...
            this.storeFT = this.syncService.getStoreClient("FT_Switches", String.class, String.class);
    ...
    public void setSwitchRole(OFControllerRole role, String swId) {
        ...
        if (reply != null) {
            logger.info("DEFINED {} as {}, reply.getRole:{}!", new Object[] { sw.getId(), role, reply.getRole() });
        } else
    ...
    public void disconnectedNode(Short nodeId) {
        ...
        String nodeIp = ClusterInfo.getControllerIp(nodeId);
        if (swIds != null) {
            String swId[] = swIds.split(",");
            for (int i = 0; i < swId.length; i++) {
                setSwitchRole(OFControllerRole.ROLE_MASTER, swId[i]);
                if (ControllerRoleManager.changeSwitchRoleToSlaveInConfigFile(nodeIp, swId[i])) {
                    logger.error("tank# change switch role to ROLE_SLAVE in remote controller { }'s config file failed",
                            nodeIp);
                } else {
                    logger.info("tank# change switch role to ROLE_MASTER in remote controller {}'s config file success",
                            nodeIp);
                }
            }
        }
